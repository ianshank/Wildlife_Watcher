...
- In this workspace, Phase 2 lives under `phase2/` and Phase 3 lives under `ml-pipeline/` as the current in-repo roadmap surfaces.
- Because the repo is now Git-backed, those directories can stay in place or move into dedicated `git worktree` checkouts without changing the command surfaces.
