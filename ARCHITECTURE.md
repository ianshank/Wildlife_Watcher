...
Native test coverage includes 16 Unity tests covering class_names lookup, MQTT thumb-budget and topic-format helpers, PowerManager stubs, the pure sleep-decision policy, and core publish logic. Tests use `power_mgr_stubs.cpp` to enable testing in native environment without hardware dependencies.
